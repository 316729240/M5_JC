...
$M.dialog = function (S) {
    ...
    win = $(document.body).addControl({
        ...
        onBeginSubmit: function () {
            win.wait(true);
            if(!$.browser.msie)win.footer[0].enabled(false);
            if (S.onBeginSubmit) S.onBeginSubmit();
        },
        onSubmit: function (sender, e) {
            win.wait(false);
            if(!$.browser.msie)win.footer[0].enabled(true);
            rdata = e;
    ...
    T.show = function () { win.show(); if( win.footer.length>0)win.footer[0].focus(); };
    T.form = win.form;
