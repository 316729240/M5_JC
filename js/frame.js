$M.Control["SelectContainer"]=function(BoxID, S,CID) {
    var T=this;
    var A = BoxID;
    var group=$("<div class=\"btn-group\" />").appendTo(A);
    if(S.style==null)S.style={width:"100%"};
    group.css(S.style);
    var button = $("<button class=\"form-control dropdown-toggle btn btn-default btn-" + $M.Control.Constant.sizeCss[S.size == null ? 1 : S.size] + "\" ><span class=\"_text \" style=\"float:left;\" ></span><span class=\"caret\"  style=\"float:right;margin-top:8px;\"></span></button>").appendTo(group);
    var _text=button.find("._text");
    var popover = group.addControl({ 
        xtype: "Popover", 
        hideArrow:true,
        onClose: function() { group.removeClass("open"); } 
    });
    T.container =popover.container;//设置容器
...
$M.Control["Popover"] = function (BoxID, S) {
    ...
    T.show = function (obj) {
        ...
        var x = 0, y = 0;
        if (S.location == null) {
            if ((pageHeight - y1 - h1) > h2) { jiantou = "bottom"; }
            else if (y1 > h2) { jiantou = "top"; }
        ...
        if(S.hideArrow){
            var _w=obj.outerWidth()+"px";
            var _h="auto";//obj.outerHeight()+"px";
            A.css({"over-flow":"auto","width":_w,"height":_h,"border-radius":"0px 0px  3px 3px"});
        }
        A.show();
        $M.focusElement = A[0];
    };
