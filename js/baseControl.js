...
$M.BaseClass = function(S) {
    ...
    this.wait = function(flag) {
        if (flag) {
            ...
            var img = $("<img src='/assets/manager/wait.gif' style='position: absolute;width:50px' />").appendTo(T.waitBox);
    ...
    selectFiles: function(back, S) {
        ...
        var change = function() {
            ...
            for (var i = 0; i < files.length; i++) {
                ...
                if (S.kzm != null) {
                    var kzm = S.kzm.split(',');
                    var fileExt = (/[.]/.exec(files[i].name)) ? /[^.]+$/.exec(files[i].name.toLowerCase()) : '';
                    if (kzm.indexOf(fileExt) > -1) {
                        json[i] = { title: files[i].name, size: files[i].size, data: files[i] };
                    } else {
                        i = files.length;
                        $M.alert("只允许上传“"+S.kzm+"”文件");
                    }
                }
            }
            if (back) back(json);
        ...
        var addFileC = function() {
            inputFile = $("<input type='file' id='" + fileId + "' name=fileData accept='image/*' " + multiple + " style='display:none' >");
            if (S && S.accept) inputFile.attr("accept", S.accept);
            button.attr("for", fileId);
            $(document.body).append(inputFile);
...
$M.Control["TagsInput"]=function(BoxID,S){
    var T = this;
    if (S.ico == null) S.ico = "fa-ellipsis-h";
    var html = '<div class="input-group btn-group">' +
                "<div class='form-control M5_tagsinput'></div>"+
            //    '<input class="inputbox form-control" ' + (S.inputReadOnly ? 'readonly' : '') + ' type="text">' +
				'<div class="input-group-btn"><button type="button" class="btn btn-default dropdown-toggle"><i class="fa ' + S.ico + '"></i></button></div>' +
			'</div>';
    A=$(html);
    ...
    var button=A.find("button");
    if (S.onButtonClick) button.click(function() { S.onButtonClick(T, null); });
    var tagbox=A.find(".M5_tagsinput");
    ...
    T.addItem=function(data){
        if (data) {
            ...
            if (typeof (data[0]) == "object") {
                ...
                var label=$('<span class="tag label label-info">'+data.text+'<span data-role="remove" class="removetag"></span></span>');
                tagbox.append(label);
                label.attr("val",data.value);
                label.attr("text",data.text);
                label.find(".removetag").click(function(){
                    label.remove();
                });
            }
        }
    ...
    T.val=function(data){
        if(data)T.addItem(data);
        var value=[];
        tagbox.find(".label-info").each(function(){
            value.push({
                text:$(this).attr("text"),
                value:$(this).attr("val")
            });
...
$M.Control["DatePickerInput"] = function(BoxID, S, CID) {
    ...
    var popover = $(document.body).addControl({ xtype: "Popover", onClose: function() { button.removeClass("open"); } });
    ...
    T.attr = function(a, b) {
        ...
        if (a == "minValue")input.attr("minDate",b);
        if (a == "maxValue")input.attr("maxDate",b);
        return S[a];
    };
    //isDate:true,
    if (S.vtype) {
        
        if (typeof (S.vtype) == "string") { 
            S.vtype=eval("S.vtype="+S.vtype);
            input.attr(S.vtype); 
        }
    }else{S.vtype={};}
    S.vtype.isDate=true;
    if(S.minValue)S.vtype.minDate=S.minValue;
    if(S.maxValue)S.vtype.minDate=S.maxValue;
    input.attr(S.vtype); 
    if (S.value) T.val(S.value);
};
...
$M.Control["SelectTree"]=function(BoxID, S,CID){
    var T=this;
    var A = null;
    var input=$("<input type=hidden name='"+S.name+"' />").appendTo(BoxID);
    var selectContainer=BoxID.addControl({xtype:"SelectContainer",style:S.style});
    var tree=selectContainer.addControl({
        xtype:"TreeView",
        onItemClicked:function(sender,e){
            T.val(e.item.attr("text"));
            selectContainer.close();
        }
    });
    T.addItem=function(items){
        tree.addItem(items);
    ...
    T.val=function(value){
        if(value!=null){
            tree.root.find(function(item){
                if(item.attr("value")==value || item.attr("text")==value){
                    selectContainer.val(item.attr("text"),item.attr("value"));
                    item.focus();
                    return;
                }
            });
        }
        value=tree.selectedItem==null?"":tree.selectedItem.attr("value")
        input.val(value);
        return value;
    };
    if(S.items)T.addItem(S.items);
    if(S.value)T.val(S.value);
};
$M.Control["TreeView"] = function(BoxID, S) {
    ...
    var A = $("<div class=\"M5_tree\" tabindex=\"-1\" ></div>").appendTo(BoxID);
    ...
    var treeChild = function(BoxID, items) {
        ...
        T2.item = function(S2) {
            ...
            _title.mouseup(function(e) {
                ...
                if (S.onItemClicked) { S.onItemClicked(T, { item: T3 }); };
            });
        ...
        var tableCell = function(S3) {
            ...
            T3.focus = function() {
                if (S.columns[S3.cellIndex].xtype && S.columns[S3.cellIndex].xtype != "CheckBox") {
                    ...
                    var xy = td.offset();
                    S3.style = { position: "fixed", zIndex: $M.zIndex++, left: xy.left + "px", top: (xy.top - document.body.scrollTop) + "px", width: td.outerWidth() + "px", height: td.outerHeight() + "px" };
                    S3.onBlur = focusBlur;
                ...
                if (S.columns[S3.cellIndex].isLink) if (S.onRowCommand) S.onRowCommand(T, {row:T2, commandName: "link", x: td.attr("_x"), y: td.parent().attr("_y") });
            ...
            td.click(function() {
                if (S.onCellMouseClick) S.onCellMouseClick(T, {row:T2,cell:T3, x: td.attr("_x"), y: td.parent().attr("_y"), target: event.target });
                return false;
            });
        };
