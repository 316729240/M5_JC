...
$M.comm = function (command, back, errback) {
  ...
  var runComm = function (appName, data, _back, _errback, index) {
      ...
      $M.ajax(url, data, function (json) {
          if (index != null) commandList[index] = json.data;
          if (json.status ==1) {
              if (_back) _back(json.data);
          } else {
              if(json.status==-1000){location.href="/index.php";return;}
              if (index != null) errdList[errdList.length] = { index: index, code: json.code, message: json.message };
              if (_errback) {
                  _errback(json);
              } else {
                  $M.alert(json.message,function(){
                      if(json.url!=null)location.href=json.url;
                  });
              }
          }
      }, function (json) {
